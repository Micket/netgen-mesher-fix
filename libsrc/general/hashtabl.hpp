...
public:
 ...
 ///
  ...
  public:
    ...
    void operator++ (int)
    {
      // mycout << "begin Operator ++: bagnr = " << bagnr << " -  pos = " << pos << endl;
      pos++;
	     ...
	     pos == ht.GetBagSize(bagnr+1))
	{
      ...
      // mycout << "end Operator ++: bagnr = " << bagnr << " - pos = " << pos << endl;
    }
...
{
  for (typename INDEX_2_HASHTABLE<T>::Iterator it = ht.Begin(); it != ht.End(); it++)
...
{
  for (typename INDEX_3_HASHTABLE<T>::Iterator it = ht.Begin(); it != ht.End(); it++)
      ost << ht.GetHash(it) << ": " << ht.GetData(it) << std::endl;
  return ost;
}
